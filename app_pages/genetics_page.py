...
def render():
    ...
    st.sidebar.markdown('<div style="font-family:IBM Plex Sans,sans-serif;font-size:0.62rem;font-weight:600;letter-spacing:0.16em;text-transform:uppercase;color:#94a3b8;margin-bottom:8px;">🧬 genetics Filters</div>', unsafe_allow_html=True)

    snapshots = list_raw_snapshots(MODULE)

    if not snapshots:
        st.warning("No genetics snapshots found. Run: python -m scripts.run_genetics_backend")
        st.stop()
    ...
    <div style="background:linear-gradient(118deg,#0c1f4a 0%,#1e3a8a 52%,#1d4ed8 100%);border-radius:24px;padding:34px 40px 30px 40px;margin-bottom:28px;position:relative;overflow:hidden;box-shadow:0 20px 60px rgba(12,31,74,0.32);">
        <div style="display:inline-flex;align-items:center;gap:7px;background:rgba(255,255,255,0.10);border:1px solid rgba(255,255,255,0.18);border-radius:999px;padding:4px 14px;font-size:0.70rem;font-weight:600;letter-spacing:0.14em;text-transform:uppercase;color:#bfdbfe;margin-bottom:14px;">
            ...
            Genetics · Missing Report Tracker
        </div>
        <div style="font-family:Syne,sans-serif;font-size:2.05rem;font-weight:800;color:#fff;line-height:1.15;letter-spacing:-0.02em;margin-bottom:8px;">🔬 Genetics Report Dashboard</div>
        <div style="font-size:0.90rem;color:rgba(186,219,255,0.82);max-width:640px;line-height:1.65;font-weight:300;">Track missing genetics reports, monitor weekly backlog trends, and identify hospitals with outstanding records across cohorts.</div>
    ...
    with c3: kpi("Missing reports",               overall_missing, "c-red",    "⚠️",   note="Outstanding genetics records")
    ...
    with col_l:
        ...
        st.markdown('<p style="font-size:0.75rem;color:#94a3b8;font-style:italic;margin-top:4px;">Backlog shows total patients missing genetics reports each week.</p>', unsafe_allow_html=True)

    with col_r:
        st.markdown('<div style="background:#ffffff;border-radius:20px;padding:16px 20px 8px 20px;border:1px solid rgba(203,213,225,0.55);box-shadow:0 4px 20px rgba(30,58,138,0.06);margin-bottom:4px;"><div style="font-family:Syne,sans-serif;font-size:0.92rem;font-weight:700;color:#0c1f4a;margin-bottom:2px;">🏥 Top 10 Hospitals by Backlog</div><div style="font-size:0.75rem;color:#94a3b8;margin-bottom:6px;">Hospitals with the most outstanding missing genetics reports</div></div>', unsafe_allow_html=True)
