"""
Biopsy Backend Pipeline

Purpose
-------
This pipeline identifies RADAR patients who are expected to have a biopsy
...
from lib.queries import SQL_BIOPSY_MISSING, SQL_BIOPSY_TOTAL_ELIGIBLE
...
def run_missing_query(conn) -> pd.DataFrame:
    ...
    df = pd.read_sql(SQL_BIOPSY_MISSING, conn)
...
def run_backend():
    ...
    try:
        ...
        total_eligible = run_scalar_query(conn, SQL_BIOPSY_TOTAL_ELIGIBLE)
