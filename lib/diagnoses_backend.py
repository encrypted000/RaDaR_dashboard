"""
Diagnoses Backend Pipeline

Purpose
-------
This pipeline identifies RADAR patients who have no diagnosis recorded at all.

Steps
-----
1. Connect to RADAR database through SSH tunnel
2. Run SQL query to retrieve patients missing diagnoses
...
Run
---
python -m scripts.run_diagnoses_backend
"""
...
from lib.queries import SQL_DIAGNOSES_MISSING, SQL_DIAGNOSES_TOTAL_ELIGIBLE
...
def get_previous_snapshot_csv(raw_dir: Path, module: str = MODULE) -> Path | None:
    """Return second most recent snapshot so we never compare a file against itself."""
...
def run_missing_query(conn) -> pd.DataFrame:
    df = pd.read_sql(SQL_DIAGNOSES_MISSING, conn)
...
def write_excel_report(
    ...
    for ci, h in enumerate(["Metric", "Value", "Details", "Status"], start=1):
        cell = ws_sum.cell(row=4, column=ci, value=h)
        cell.font = hdr_font(size=10); cell.fill = fill("1A3A8F")
        cell.alignment = centre(); cell.border = border
    ws_sum.row_dimensions[4].height = 22

    kpi_rows = [
        ("Total Eligible Patients",  total_eligible,       "All RADAR patients",            BLUE_BG),
        ("Diagnoses Recorded",       uploaded,             "At least one diagnosis on file", GREEN_BG),
        ("Missing Diagnoses",        missing_total,        "No diagnosis recorded",          RED_BG   if missing_total > 0 else GREEN_BG),
        ...
        ("Resolved (this week)",     len(resolved_ids),    "Diagnosis now recorded",         GREEN_BG),
    ...
    for ri, (metric, value, detail, bg) in enumerate(kpi_rows, start=5):
        ...
        status_txt = "⚠ Action needed" if bg == RED_BG else ("✓ OK" if bg == GREEN_BG else "→ Monitor")
        ws_sum.cell(row=ri, column=4, value=status_txt).font = body_font(bold=True)
        ws_sum.cell(row=ri, column=4).fill = fill(bg)
    ...
    def write_patient_sheet(wb, title, df, status_filter=None, hdr_color="1A3A8F",
                             new_color=RED_BG, still_color=AMBER_BG):
        ...
        for ci, h in enumerate(headers, start=1):
            ...
            cell.font = hdr_font(size=10); cell.fill = fill(hdr_color)
            cell.alignment = centre(); cell.border = border
        ...
        for ri, (_, row) in enumerate(df[display_cols].iterrows(), start=2):
            ...
            for ci, col in enumerate(display_cols, start=1):
                ...
                cell.font = body_font(); cell.fill = fill(row_bg)
    ...
    ws_res["A1"].font = hdr_font(size=11); ws_res["A1"].fill = fill("276221")
    ws_res["A1"].alignment = centre(); ws_res.row_dimensions[1].height = 24

    for ci, h in enumerate(["Patient ID", "Note"], start=1):
        ws_res.cell(row=2, column=ci, value=h).font = hdr_font(size=10)
        ws_res.cell(row=2, column=ci).fill = fill("1A3A8F")
        ws_res.cell(row=2, column=ci).alignment = centre()
        ws_res.cell(row=2, column=ci).border = border

    for ri, pid in enumerate(sorted(resolved_ids), start=3):
        bg = GREEN_BG if ri % 2 == 0 else "EAFBEA"
        ws_res.cell(row=ri, column=1, value=pid).font = body_font()
        ws_res.cell(row=ri, column=1).fill = fill(bg)
        ws_res.cell(row=ri, column=1).alignment = left()
        ws_res.cell(row=ri, column=1).border = border
        ws_res.cell(row=ri, column=2, value="Diagnosis now on file").font = body_font(color="276221")
        ws_res.cell(row=ri, column=2).fill = fill(bg)
...
def run_backend():
    ...
    try:
        ...
        total_eligible = run_scalar_query(conn, SQL_DIAGNOSES_TOTAL_ELIGIBLE)
    finally:
        conn.close()
        tunnel.stop()

    missing_total = df_missing["patient_id"].nunique()
    uploaded      = max(0, total_eligible - missing_total)
    ...
    if "age" in df_missing2.columns:
        ...
        adult_missing = child_missing = 0

    snap_csv      = raw_dir / f"missing_{MODULE}_{runstamp}.csv"
    _expected     = ["patient_id", "age", "hospital_name", "hospital_code", "cohort_group", "recruited_date", "status"]
    _save_cols    = [c for c in _expected if c in df_missing2.columns]
    ...
    update_weekly_kpis(
        kpis_dir, week_monday_key(),
        total_eligible, uploaded, missing_total, completeness,
        len(new_ids), len(still_ids), len(resolved_ids),
        adult_missing, child_missing,
    )

    for _old in sorted(raw_dir.glob(f"missing_{MODULE}_*.csv"),     reverse=True)[12:]:
        _old.unlink()
    for _old in sorted(reports_dir.glob(f"{MODULE}_missing_report_*.xlsx"), reverse=True)[12:]:
        _old.unlink()

    print("=" * 50)
    print(f"  Module          : {MODULE}")
    print(f"  Total eligible  : {total_eligible}")
    print(f"  Diagnoses on file: {uploaded}")
    ...
    print("=" * 50)
    print("Saved snapshot :", snap_csv)
    print("Saved report   :", excel_path)
